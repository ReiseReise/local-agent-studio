...
> 当前状态：`0.1.0-alpha`。Windows 11 虚拟机中的核心安装与启停流程已验证；模型实调、Siver 接口和真实联系人试运行仍必须分别验收。
...
| M1 基础服务 | Windows 已部署，模型鉴权待用户完成 | 数据库迁移、模型配置、密钥保护、健康检查已自动测试；真实上游已到达但尚未取得成功回答 |
...
| M6 GitHub 发布 | 未开始 | 公开建仓、推送和 Release 前再次确认 |
...
- Windows 当前配置的 Agent、聊天模型、已发布提示词和知识库健康检查均已就绪；真实模型请求已到达上游，但返回 401 鉴权失败，须由用户本人重新填写有效 API Key 后再验收成功回答。
